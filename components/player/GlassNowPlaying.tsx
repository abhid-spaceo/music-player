...
import styles from './GlassNowPlaying.module.css';

type PlayStats = { count: number; lastPlayedAt: string | null };
type StatsState = (PlayStats & { forId: string }) | null;

/**
 * Glass variant of Now Playing. Behaviour is identical to NowPlaying (same player
 * hooks, the same hash-history close, the same stats fetch and drag-to-reorder) —
 * only the presentation differs: floating frosted panels over the per-track art
 * bloom (--art-glow, set on <html> by useArtGlow in AppShell). Rendered instead
 * of NowPlaying when the theme is glass; the Current overlay is untouched.
 */
export function GlassNowPlaying() {
  const {
  ...
  // Make the overlay a real history entry so Back / Esc / the ⌄ arrow closes it
  // instead of leaving the app. (Identical to NowPlaying — see its comment.)
  useEffect(() => {
    if (!expanded) return;
  ...
  return (
    <div className={styles.overlay} role="dialog" aria-label="Now playing">
      {/* Ambient bloom, tinted from the current artwork (falls back to preset). */}
      ...
      <div className={styles.stage}>
        <div className={styles.artColumn}>
          ...
          <div className={styles.transportPanel}>
            ...
            <div className={styles.transport}>
              ...
              <button type="button" className={styles.ctl} onClick={previous} aria-label="Previous track">
                ⏮
              </button>
        ...
        <section className={styles.queueColumn} aria-label="Up next">
          <h2 className={styles.queueLabel}>UP NEXT · {upNext.length}</h2>
